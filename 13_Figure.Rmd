...
## Panel I : Gene Expression Pattern Plots
```{r}
#Assigning the data
pattern.weights<- as.matrix(cogapsRes@sampleFactors[, colnames(cogapsRes@sampleFactors) %in% c("Pattern_5","Pattern_8","Pattern_18", "Pattern_14")])
colnames(pattern.weights) <- c("Pattern 5","Pattern 8","Pattern 14", "Pattern 18")

day.info <- matrix(cdsNorm@colData@listData$day)
colnames(day.info) <- "Day"
rownames(day.info) <- rownames(cdsNorm@colData)
...
expression.data <- as.matrix(log10(expression.data))

data<-cbind(pattern.weights, expression.data[rownames(pattern.weights), ], day.info[rownames(pattern.weights), ])
data<- as.data.frame(data)
names(data)[12] <- "Day"


#Test Plot with 1 gene and 1 pattern
ggplot(data,aes(as.numeric(Pattern_5),as.numeric(Vim))) + geom_point(aes(colour=Day)) + labs(y="log10(Expression)",title="Vim",x="Pattern 5") + theme(axis.text = element_blank(),)+ geom_smooth(color='black') + theme_minimal() + theme(plot.title = element_text(hjust = 0.5))
...
#Generating pdf of Pattern 5
  ...
  for (i in 1:ncol(loop1)){
    ...
    print(ggplot(data,aes(as.numeric(Pattern_5),as.numeric(loop1[,i]))) + geom_point(aes(colour=Day)) + labs(title=colnames(loop1[i]), x="Pattern 5", y="log10(Expression)") + theme(axis.text = element_blank())+ geom_smooth(color='black',se=FALSE) + theme_minimal() + theme(plot.title = element_text(hjust = 0.5)))
...
#Generating pdf of Pattern 8
  ...
  for (i in 1:ncol(loop1)){
    ...
    print(ggplot(data,aes(as.numeric(Pattern_8),as.numeric(loop1[,i]))) + geom_point(aes(colour=Day)) + labs(title=colnames(loop1[i]), x="Pattern 8", y="log10(Expression)") + theme(axis.text = element_blank())+ geom_smooth(color='black',se=FALSE) + theme_minimal() + theme(plot.title = element_text(hjust = 0.5)))
  ...
  for (i in 1:ncol(loop1)) {
    ...
    print(ggplot(data,aes(as.numeric(Pattern_14),as.numeric(loop1[,i]))) + geom_point(aes(colour=Day)) + labs(title=colnames(loop1[i]), x="Pattern 14", y="log10(Expression)") + theme(axis.text = element_blank())+ geom_smooth(color='black',se=FALSE) + theme_minimal() + theme(plot.title = element_text(hjust = 0.5)))
  ...
  for (i in 1:ncol(loop1)){
    ...
    print(ggplot(data,aes(as.numeric(Pattern_18),as.numeric(loop1[,i]))) + geom_point(aes(colour=Day)) + labs(title=colnames(loop1[i]), x="Pattern 18", y="log10(Expression)") + theme(axis.text = element_blank())+ geom_smooth(color='black',se=FALSE) + theme_minimal() + theme(plot.title = element_text(hjust = 0.5)))
